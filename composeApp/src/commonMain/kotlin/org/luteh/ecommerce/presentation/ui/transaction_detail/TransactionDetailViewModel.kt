...
class TransactionDetailViewModel :
    BaseViewModel<
        ...
        TransactionDetailViewModel.Event,
        TransactionDetailViewModel.Effect,
    >(State()) {

    data class State(val isSuccess: Boolean = false, val message: String = "")
    ...
    override fun processEvent(event: Event) {
        when (event) {
            is Event.Init ->
                updateState { it.copy(isSuccess = event.isSuccess, message = event.message) }
